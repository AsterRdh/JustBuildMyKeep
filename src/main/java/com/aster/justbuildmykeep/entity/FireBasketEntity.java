...
public class FireBasketEntity extends TileEntity implements IClearable, ITickableTileEntity {
    private final NonNullList<ItemStack> inventory = NonNullList.withSize(4, ItemStack.EMPTY);
    private final int[] cookingTimes = new int[4];
    private final int[] cookingTotalTimes = new int[4];

    public FireBasketEntity() {
        super(TileEntityTypeRegistry.obsidianTERTileEntity.get());
    }
    public void tick() {
        boolean flag = this.getBlockState().get(FireBasket.LIT);
        ...
        if (world != null) {
            ...
            if (random.nextFloat() < 0.11F) {
                for(int i = 0; i < random.nextInt(2) + 2; ++i) {
                    FireBasket.spawnSmokeParticles(world, blockpos, this.getBlockState().get(FireBasket.SIGNAL_FIRE), false);
                }
            }

            int l = this.getBlockState().get(FireBasket.FACING).getHorizontalIndex();
    ...
    public void read(BlockState state, CompoundNBT nbt) {
        ...
        if (nbt.contains("CookingTimes", 1)) {
